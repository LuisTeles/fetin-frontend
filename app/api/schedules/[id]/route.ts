...
async function deleteSchedule(accessToken: string, id: string, impersonateUserId?: string | null) {
    const headers: Record<string, string> = {
        Authorization: `Bearer ${accessToken}`,
    }
    if (impersonateUserId) {
...
export async function DELETE(
    ...
    let response = await deleteSchedule(accessToken, id, userId)
    ...
    if (response.status === 401) {
        ...
        response = await deleteSchedule(refreshed.accessToken, id, userId)
    }
